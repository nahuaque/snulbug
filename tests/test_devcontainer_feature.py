...
def test_codespace_local_gateway_example_documents_member_agent_flow(monkeypatch):
    ...
    assert loaded_env_config["audit_out"] == example / "traces/codespace-env-audit.jsonl"
    ...
    assert "snulbug mcp codespace serve-demo" in readme
    assert "snulbug mcp codespace attach" in readme
