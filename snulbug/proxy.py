...
class ReverseProxyApp:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if not completion_allowed and isinstance(request, Mapping):
            ...
            return
        progress_allowed, progress_request_metadata = enforce_mcp_progress_request_policy(
            request,
            config=self.progress_policy,
            state_store=self.protocol_state_store,
        )
        if not progress_allowed and isinstance(request, Mapping):
            response = mcp_progress_error_response(request, progress_request_metadata)
            _set_proxy_metadata(
                scope,
                {
                    **_mcp_request_metadata(request),
                    ...
                    **_completion_policy_metadata(completion_metadata),
                    "protocol_policy": progress_request_metadata,
                },
            )
            await _send_response(
                send,
...
class McpFacadeProxyApp:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            await self._forward_to_default(
                routes,
                scope,
                body,
                send,
...
def create_proxy_application(
    ...
    *,
    ...
    progress_policy_action: str = "warn",
...
def run_proxy(
    *,
    ...
    app = create_proxy_application(
        ...
        progress_policy_action=progress_policy_action,
        progress_rate_limit=progress_rate_limit,
        progress_rate_window_seconds=progress_rate_window_seconds,
...
def proxy_config_run_kwargs(
    ...
    *,
    ...
    return {
        ...
        "progress_policy_action": proxy_config["progress_policy_action"],
...
def _oauth_resource_config_from_normalized(normalized: Mapping[str, Any]) -> OAuthResourceConfig:
    return OAuthResourceConfig(
        ...
        jwks_url=normalized["jwks_url"],
        jwks_cache_seconds=normalized["jwks_cache_seconds"],
        jwks_fetch_timeout=normalized["jwks_fetch_timeout"],
        issuer_metadata_url=normalized["issuer_metadata_url"],
        issuer_discovery=normalized["issuer_discovery"],
        token_validation=normalized["token_validation"],
        introspection_endpoint=normalized["introspection_endpoint"],
        introspection_client_id=normalized["introspection_client_id"],
        introspection_client_secret_env=normalized["introspection_client_secret_env"],
        introspection_cache_seconds=normalized["introspection_cache_seconds"],
        introspection_fetch_timeout=normalized["introspection_fetch_timeout"],
...
def _progress_policy_metadata(
    request_metadata: Mapping[str, Any],
    response_metadata: Mapping[str, Any],
) -> dict[str, Any]:
    merged: dict[str, Any] = {}
    if request_metadata.get("checked") or request_metadata.get("request") or request_metadata.get("tracked_request"):
